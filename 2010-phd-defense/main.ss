...
(define (thesis-slide)
  (define-syntax-rule (maybe-red stg txt)
    (pict-case stage-name
               [(stg) (red (t txt))]
               [else (t txt)]))
  (parameterize ([current-para-width 700])
    (slide/stage
     [thesis mod sister easy]
     #:layout 'center #:title "Thesis"
     (italic
      (para 
       (maybe-red mod "Module-by-module")
       "porting of code from an untyped language to a"
       (maybe-red sister "typed sister language") 
       "allows for an" 
       (maybe-red easy "easy transition")
       "from untyped"
...
(slide #:title "Support"
       (para "Sound Typed-Untyped Interoperation")
       (para "Type System for Scheme")
       (para "Full-scale Implementation")
       (para "Empirical Validation"))

(thanks)

(slide #:title "Proposal"
       (para "Occurence Typing")
       (subitem "Done, and more")
       (para "Variable-arity Polymorphism")
       (subitem "Done")
       (para "Keyword and Optional Arguments")
       (subitem "Done: Use & Import/Export")
       (subitem "Not Done: Definition")
       (para "Validation")
       (subitem "Some Done"))
