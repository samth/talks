...
(require slideshow/play (except-in "beamer.ss" title) "lib.ss")

(define (thank . names)
  (for/list ([n names])
    (list (t (string-append "Thanks to " n)))))

(define all-names (list "Matthias Felleisen" "Ryan Culpepper" "Stevie Strickland" "Ivan Gazeau" "Carl Eastlund" "Matthew Flatt"
...
(define (thanks [names all-names])
  ...
  (play-n #:steps (sub1 (* (length names) 1))
          #:delay 0.5
          #:skip-first? #t
