...
(define (mod/lang lang 
  ...
  (define box
    (vl-append
     (hbl-append (if (string? hlang) ((current-code-tt) hlang) hlang) ((current-code-tt) lang))
     (if space (code ||) (blank 1))
     (lt-superimpose
      (ghost sz)
...
(define-syntax-rule (bmod lang . args)
  (transparent-block (mod/lang lang #:name-frame (name-back "black") . args)))

(define-syntax-rule (cmod . args)
 (parameterize ([current-outline-color "gray"]) 
   (transparent-block (mod/lang "" #:hlang (colorize ((current-code-tt) "COMPILED") "gray") #:name-frame (name-back "gray") . args))))
