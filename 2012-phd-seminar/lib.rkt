...
(define (mod/lang lng 
  ...
  (define box
    (vl-append
     ...
     (lt-superimpose
      (ghost sz)
      (ghost (blank (if (number? width) width (pict-width width)) height))
      (apply
       vl-append 
       body))))
  (if name
...
(define (highlight-on big #:inset [n 2] . ps)
  (parameterize ([current-block-inset n])
...
(define (connect big p1 p2 [inset 15] #:find1 [find2 #f] #:find2 [find1 #f])
  ...
  (pin-arrows-line #:hide-arrowhead? #t #:line-width 4 8 big2 h1 (or find2 (find/adapt h2 #f)) h2 (or find1 (find/adapt h1 #t))
                   #:start-angle 0 #:end-angle (/ pi -6)
                   ))
...
(define-syntax-rule (red-code . e)
  (parameterize ([code-colorize-enabled #f])
    (colorize (code . e) "red")))
(define-syntax-rule (black-code . e)
  (parameterize ([code-colorize-enabled #f])
    (colorize (code . e) "black")))
...
(define (white-box p)
  (define p* (inset p 8))
  (cc-superimpose (rectangle/border (pict-width p*) (pict-height p*) #:color "white" #:border-color "red") p*))