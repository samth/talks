...
(require unstable/gui/slideshow scheme/class slideshow #;"t-slideshow.ss"
         (except-in "beamer.ss" title) "lib.ss" scheme/gui/base)
...
(define (textify t font size)
  (if (pict? t)
      t
      (if (list? t)
          (apply hbl-append (for/list ([i t]) (textify i font size)))
          (text t font size))))
...
(define (tslide t . subs)
  ...
  (parameterize ([current-background-pict (bitmap plt-title-background-path)])
    (slide #:layout 'center
           (if (null? subs)
               (textify t (current-title-font) title-text-size)
               (apply vc-append 0 
                      (append 
