...
(author "Sam Tobin-Hochstadt and Matthias Felleisen" "Northeastern University" #f)
...
(define t-sq (code (define: (sq [x : Num]) : Num 
                     (* x x))))

(define t-sq/prov (code #,t-sq
                        (provide sq)))

(define s-sq (lt-superimpose (ghost t-sq) (code (define (sq x) 
                                                  (* x x)))))
...
(define (fn #:circle? [circle? (code circle?)]
            ...
            #:typed [typed #f])
  (code (#,(if typed (code define:) (code define)) #,(if typed (code (shape-area [s : Shape]) : Num) (code (shape-area s)))
          (cond
            [(position? s) 0]
  ...
  (code 
   #,(if typed          
         ...
         (code (code:comment "Shape = Position U Circle U Rectangle U ... ")
               ||))   
   #,(if typed (code ||) (code (code:contract Shape -> Num)))
