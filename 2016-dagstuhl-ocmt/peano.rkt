...
(require slideshow "lib.ss" (except-in unstable/gui/slideshow shade) slideshow/code "config.rkt")
...
(define tZ (code 'Z))
(define tS (code (Cons 'S Peano)))
(define zero (code 0))
(define sym? (code (symbol? #,n2)))
(define els (code (add1 (convert (rest #,n3)))))

(define dt (code (define-type Peano (U #,(launder tZ) #,(launder tS)))))
...
(define pic0
     (pin-over (ghost pic) dt lt-find dt))
...
(define (peano1)
  (staged 
   ...
   (define (build n ty)
     (define ty* (code n : #,ty))
     (connect (mini-slide (shaded pic) (blank 50)
                          (pict-if #t ty* (launder (inf-rule (code x : #,(overbar tS))
                                                             (code x : #,tS)))))
              n ty*)) 
   (slide (pict-case stage-name
            [(zero*) pic0]
            [(one) pic]
            [(two) (build n1 tN)]
            [(three) (build n2 tN)]
