...
(require slideshow "lib.ss" 
         (only-in slideshow/slide get-margin title-size line-sep)
         (except-in "beamer.rkt" title) (except-in unstable/gui/slideshow shade) slideshow/code "config.rkt")
  ...
  (staged 
   ...
   (define (build n ty)
     ...
     (connect (mini-slide (shaded pic) (blank 50) (pict-if #t ty* (launder (code x : #,tS))))
              n ty*)) 
   (slide #:title (case stage
                    [(1) "Occurrence Typing"] 
                    [else (shaded (make-title-text "Occurrence Typing"))]) 
