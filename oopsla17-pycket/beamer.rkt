#lang racket
(require slideshow/code racket/gui (except-in slideshow struct struct/ctc make-base-namespace make-base-empty-namespace)
         scheme/runtime-path
 ...
 (lambda ()
   (slide #:layout 'center
    (parameterize ((current-block-background-color "blue"))
      (block 
       (inset (colorize (vc-append (if (string? (title))
                                       (text (string-append (title) "")
                                             (current-main-font) title-size)
                                       (title))
                                   (if (subtitle) 
